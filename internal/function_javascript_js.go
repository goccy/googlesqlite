...
// This is the js/wasm build of the JavaScript UDF evaluator. It uses goja
// (pure Go) because modernc.org/quickjs — used by the !js build — depends
// on modernc.org/libc, which has no GOOS=js port. goja reaches
// reflect.Value.MethodByName, so the js/wasm binary does not benefit from
// the dead-code-elimination win the native build gets; replacing goja
// here needs a JS engine that builds for js/wasm (e.g. a wasm2go
// transpilation of QuickJS).
