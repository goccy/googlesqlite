...
testdata: testdata/specs/googlesql/syntax/ddl/create_schema.yaml
---
...
Creates a dataset (`CREATE SCHEMA <name> [OPTIONS(...)]`). Acceptance-only in googlesqlite — schemas materialise lazily when the first table is created under them.
